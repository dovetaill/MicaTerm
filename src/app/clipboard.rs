...
pub(crate) fn system_clipboard_image_source() -> Result<Option<ClipboardImageSource>> {
    use clipboard_win::formats::{Bitmap, FileList};
    use clipboard_win::{Clipboard, Format, Getter};

    let bitmap_available = Bitmap.is_format_avail();
    let file_list_available = FileList.is_format_avail();
    if !bitmap_available && !file_list_available {
        return Ok(None);
    }
    ...
    if Bitmap.is_format_avail() {
        ...
        return Ok(Some(ClipboardImageSource::Bitmap(bitmap)));
    }
...
pub(crate) fn encode_clipboard_image(
    ...
    match source {
        ClipboardImageSource::Bitmap(bytes) => encode_with_reader_factory(|| {
            ImageReader::new(Cursor::new(bytes.as_slice()))
                .with_guessed_format()
                .context("failed to inspect the clipboard bitmap format")
        }),
