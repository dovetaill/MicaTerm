...
impl SftpBrowserSessionState {
    ...
    pub fn set_loading_follow(&mut self, path: &str, generation: u64, request_id: u64) {
        self.follow_mode = SftpFollowMode::FollowCwd;
        self.current_path = path.to_string();
        self.history.push(path.to_string());
        self.restore_cached_entries(path);
    ...
    pub fn set_loading_manual(&mut self, path: &str, generation: u64, request_id: u64) {
    ...
    pub fn navigate_back(&mut self, generation: u64, request_id: u64) -> Option<String> {
        let path = self.history.back()?.to_string();
        self.begin_manual_navigation(path.clone(), generation, request_id, false);
        Some(path)
    }

    pub fn navigate_forward(&mut self, generation: u64, request_id: u64) -> Option<String> {
        let path = self.history.forward()?.to_string();
        self.begin_manual_navigation(path.clone(), generation, request_id, false);
        Some(path)
    }

    pub fn navigate_up(&mut self, generation: u64, request_id: u64) -> Option<String> {
        let path = remote_parent_path(self.current_path.as_str())?;
        self.begin_manual_navigation(path.clone(), generation, request_id, true);
        Some(path)
    }
    ...
    pub fn set_retrying(&mut self, path: &str, generation: u64, request_id: u64) {
        self.current_path = path.to_string();
        self.restore_cached_entries(path);
        self.mode = SftpPanelMode::Connecting;
        self.last_error = None;
    ...
    pub fn accepts_request(&self, generation: u64, request_id: u64) -> bool {
        self.generation == generation && self.active_request_id == Some(request_id)
    }

    fn begin_manual_navigation(
        &mut self,
        path: String,
        generation: u64,
        request_id: u64,
