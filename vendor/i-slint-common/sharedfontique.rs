...
const BUILTIN_UI_FONT_BYTES: &[u8] = include_bytes!("sharedfontique/DejaVuSans.ttf");

pub static COLLECTION: std::sync::LazyLock<Collection> = std::sync::LazyLock::new(|| {
    create_collection(false)
});

pub static SYSTEM_COLLECTION: std::sync::LazyLock<Collection> =
    std::sync::LazyLock::new(|| create_collection(true));

pub fn get_collection() -> Collection {
    COLLECTION.clone()
}

pub fn get_system_collection() -> Collection {
    SYSTEM_COLLECTION.clone()
}
...
fn create_collection(system_fonts: bool) -> Collection {
    ...
    register_builtin_default_font(&mut collection);
...
fn register_builtin_default_font(collection: &mut fontique::Collection) {
    let fonts =
        collection.register_fonts(fontique::Blob::new(Arc::new(BUILTIN_UI_FONT_BYTES)), None);
