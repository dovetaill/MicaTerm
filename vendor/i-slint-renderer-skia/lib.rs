...
impl SkiaRenderer {
    ...
    /// Purges retained renderer caches while keeping the current surface alive.
    pub fn purge_memory_resources(&self) -> Result<(), PlatformError> {
        self.clear_item_caches();

        if let Some(surface) = self.surface.borrow().as_ref() {
            surface.purge_memory_resources()?;
        }
...
impl i_slint_core::renderer::RendererSealed for SkiaRenderer {
    ...
    fn register_font_from_memory(
        ...
        let info_override = bundled_ui_font_info_override(&data);
    ...
    fn register_font_from_path(
        ...
        let info_override = bundled_ui_font_info_override(&contents);
...
fn bundled_ui_font_info_override(
    data: &[u8],
    ...
    match post_script_name.as_str() {
        "JetBrainsMapleMono-Regular" => Some(sharedfontique::fontique::FontInfoOverride {
