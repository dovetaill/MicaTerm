...
impl Terminal {
    ...
    /// Drop an incomplete escape sequence without replacing terminal state.
    ///
    /// Embedders can use this after enforcing a bounded control-string policy;
    /// bytes belonging to the rejected sequence must be discarded through its
    /// terminator before normal parsing resumes.
