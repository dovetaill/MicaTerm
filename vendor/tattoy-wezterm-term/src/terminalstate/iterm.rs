...
impl TerminalState {
    pub(crate) fn set_image(&mut self, image: ITermFileData) {
        if !image.inline {
            if self.config.allow_iterm2_file_downloads() {
        ...
        if image.data.len() > self.config.max_image_encoded_bytes() {
            error!(
                "Ignoring iTerm2 image with encoded size {} above configured limit {}",
                image.data.len(),
        ...
        if let Err(err) = check_image_dimensions(
            info.width,
            info.height,
        ...
        let data = match (downscaled, info.format) {
            ...
            (true, _) => match decode_image_with_limits(
                &image.data,
                self.config.max_image_pixels(),
                self.config.max_image_decoded_bytes(),
            ) {
