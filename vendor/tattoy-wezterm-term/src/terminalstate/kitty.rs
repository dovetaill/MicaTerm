...
pub struct KittyImageState {
    ...
    accumulated_encoded_bytes: usize,
    discarding_accumulation: bool,
    max_image_id: u32,
...
impl KittyImageState {
    fn clear_accumulation(&mut self) {
        ...
        self.accumulated_encoded_bytes = 0;
        self.discarding_accumulation = false;
    }
    ...
    fn record_id_to_data(
        ...
        self.prune_unreferenced(budget, data.len());
        anyhow::ensure!(
            data.len() <= budget && self.used_memory.saturating_add(data.len()) <= budget,
            "kitty image resources exceed the configured {} byte session budget",
            budget
        );
    ...
    fn prune_unreferenced(&mut self, budget: usize, required: usize) {
        if self.used_memory.saturating_add(required) > budget {
...
fn kitty_image_transmit(image: &KittyImage) -> Option<&KittyImageTransmit> {
    match image {
        KittyImage::TransmitData { transmit, .. }
        | KittyImage::TransmitDataAndDisplay { transmit, .. }
        | KittyImage::Query { transmit }
        | KittyImage::TransmitFrame { transmit, .. } => Some(transmit),
        KittyImage::Display { .. }
        | KittyImage::Delete { .. }
        | KittyImage::ComposeFrame { .. } => None,
    }
}

fn kitty_image_uses_external_media(image: &KittyImage) -> bool {
    kitty_image_transmit(image).is_some_and(|transmit| {
        matches!(
            &transmit.data,
            KittyImageData::File { .. }
...
fn kitty_image_identifiers(image: &KittyImage) -> (Option<u32>, Option<u32>) {
    kitty_image_transmit(image)
        .map(|transmit| (transmit.image_id, transmit.image_number))
        .unwrap_or((None, None))
}

fn kitty_image_chunk(image: &KittyImage) -> Option<&KittyImageTransmit> {
    match image {
        KittyImage::TransmitData { transmit, .. }
        | KittyImage::TransmitDataAndDisplay { transmit, .. } => Some(transmit),
        _ => None,
    }
}

fn direct_payload_decoded_len(data: &KittyImageData) -> anyhow::Result<usize> {
    match data {
        KittyImageData::Direct(data) => {
            ...
            let decoded = (len / 4)
                .saturating_mul(3)
                .saturating_add((len % 4).saturating_sub(1))
                .saturating_sub(padding);
            Ok(decoded)
        }
        KittyImageData::DirectBin(data) => Ok(data.len()),
        KittyImageData::File { .. }
        | KittyImageData::TemporaryFile { .. }
        | KittyImageData::SharedMem { .. } => {
            anyhow::bail!("external kitty image media cannot be accumulated")
        }
    }
}

impl TerminalState {
    fn kitty_accumulation_allows(&mut self, image: &KittyImage) -> bool {
        let Some(transmit) = kitty_image_chunk(image) else {
            return true;
        };

        if self.kitty_img.discarding_accumulation {
            if !transmit.more_data_follows {
                self.kitty_img.clear_accumulation();
            }
            return false;
        }

        let chunk_bytes = match direct_payload_decoded_len(&transmit.data) {
            Ok(bytes) => bytes,
            Err(err) => {
                let (image_id, image_number) = kitty_image_identifiers(image);
        ...
        let next_bytes = self
            .kitty_img
            .accumulated_encoded_bytes
            .saturating_add(chunk_bytes);
        let limit = self.config.max_image_encoded_bytes();
        if next_bytes > limit {
            let identifiers = self
                .kitty_img
                .accumulator
                .first()
                .map(kitty_image_identifiers)
                .unwrap_or_else(|| kitty_image_identifiers(image));
            self.kitty_img.clear_accumulation();
            self.kitty_img.discarding_accumulation = transmit.more_data_follows;
            self.kitty_send_response(
                image.verbosity(),
                false,
                identifiers.0,
                identifiers.1,
                format!("ERROR:kitty image encoded data exceeds the configured {limit} byte limit"),
            );
            return false;
        }

        self.kitty_img.accumulated_encoded_bytes = if transmit.more_data_follows {
            next_bytes
        } else {
            0
        };
    ...
    pub(crate) fn kitty_img(&mut self, img: KittyImage) -> anyhow::Result<()> {
        ...
        if kitty_image_chunk(&img).is_none() {
            self.kitty_img.clear_accumulation();
        }
        if !self.config.allow_kitty_graphics_external_media()
            && kitty_image_uses_external_media(&img)
        {
            let (image_id, image_number) = kitty_image_identifiers(&img);
            self.kitty_img.clear_accumulation();
            self.kitty_send_response(
                verbosity,
                false,
                image_id,
                image_number,
                "ERROR:external kitty image media is disabled".to_string(),
            );
            return Ok(());
        }
        if !self.kitty_accumulation_allows(&img) {
            return Ok(());
        }
        match img {
            KittyImage::Query { transmit } => {
                let image_id = transmit.image_id;
                let image_number = transmit.image_number;
                match direct_payload_decoded_len(&transmit.data).and_then(|size| {
                    anyhow::ensure!(
                        size <= self.config.max_image_encoded_bytes(),
                        "kitty image encoded data exceeds the configured {} byte limit",
                        self.config.max_image_encoded_bytes()
                    );
                    Ok(())
                })
                .and_then(|()| transmit.data.load_data().map_err(anyhow::Error::from))
                {
                    Ok(_) => {
                        self.kitty_send_response(
                            verbosity,
            ...
            KittyImage::TransmitData {
                ...
                if more_data_follows {
                    ...
                    let identifiers = self
                        ...
                        .first()
                        .map(kitty_image_identifiers)
                        .unwrap_or_else(|| kitty_image_identifiers(&img));
            ...
            KittyImage::Delete {
                ...
                self.kitty_send_response(verbosity, true, None, None, "OK".to_string());
            }
            ...
            KittyImage::TransmitFrame {
                ...
                "ERROR:animated kitty images are unsupported".to_string(),
            ),
            KittyImage::ComposeFrame { frame, verbosity } => self.kitty_send_response(
                verbosity,
                false,
                frame.image_id,
                frame.image_number,
    ...
    fn kitty_img_transmit_inner(
        ...
        anyhow::ensure!(
            data.len() <= self.config.max_image_encoded_bytes(),
            "kitty image encoded data exceeds the configured {} byte limit",
            self.config.max_image_encoded_bytes()
        );
        ...
        let data = match transmit.compression {
            ...
            KittyImageCompression::Deflate => {
                miniz_oxide::inflate::decompress_to_vec_zlib_with_limit(
                    &data,
                    self.config.max_image_decoded_bytes(),
                )
    ...
    fn kitty_img_transmit(
        ...
        self.kitty_img.record_id_to_data(
            image_id,
            img,
            self.config.kitty_graphics_max_resource_bytes(),
        )?;
