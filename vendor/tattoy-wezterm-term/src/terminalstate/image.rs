...
impl TerminalState {
    ...
    pub(crate) fn raw_image_to_image_data(
        ...
        self.image_cache.retain(|_, resource| {
            if let Some(resource) = resource.upgrade() {
                retained_bytes = retained_bytes.saturating_add(resource.len());
                true
            } else {
                false
            }
        ...
        if let Some(item) = self.image_cache.get(&key).and_then(|item| item.upgrade()) {
            return Ok(item);
        }

        let data = data.swap_out()?;
        let image_data = Arc::new(ImageData::with_data(data));
        let resource_bytes = image_data.len();
        let budget = self.config.max_image_resource_bytes();
        if resource_bytes > budget
            || self
                ...
                > budget
        {
            return Err(anyhow::anyhow!(
                "terminal image resources exceed the configured {} byte session budget",
                budget
            )
            .into());
        }
        ...
        self.image_cache_retained_bytes = self
            .image_cache_retained_bytes
            .saturating_add(resource_bytes);
        self.image_cache.insert(key, Arc::downgrade(&image_data));
        Ok(image_data)
    }
}
...
pub(crate) fn check_image_dimensions(
    ...
    let size = pixels.saturating_mul(4);
    if pixels > max_pixels || size > max_decoded_bytes as u64 {
        anyhow::bail!(
            "Ignoring image data for image with dimensions {}x{} \
             because {} pixels / required RAM {} exceeds configured limits \
             of {} pixels / {} bytes",
            width,
            height,
            pixels,
