...
impl FontRequest {
    ...
    pub fn query_fontique(&self) -> Option<i_slint_common::sharedfontique::fontique::QueryFont> {
        ...
        if let Some(font) = self.query_fontique_in_collection(&mut collection) {
            return Some(font);
        }

        let mut system_collection = sharedfontique::get_system_collection();
        self.query_fontique_in_collection(&mut system_collection)
    }

    fn query_fontique_in_collection(
        &self,
        collection: &mut i_slint_common::sharedfontique::Collection,
