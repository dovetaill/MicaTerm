...
pub fn decode_wide(w_str: *mut u16) -> String {
    // Avoid Win32_Globalization so Linux `cargo xwin` builds do not pull in `icu.dll`
    // alongside Skia's bundled ICU objects.
    let len = nul_terminated_wide_len(w_str);
    let w_str_slice = unsafe { std::slice::from_raw_parts(w_str, len) };
    String::from_utf16_lossy(w_str_slice)
}

fn nul_terminated_wide_len(w_str: *const u16) -> usize {
    if w_str.is_null() {
        return 0;
    }
