...
fn dark_theme_maps_terminal_palette_to_mica_graphite() {
    ...
    assert_eq!(preset.background, 0x11_1821);
    assert_eq!(preset.foreground, 0xe5_ebf5);
    assert_eq!(preset.cursor_bg, 0xe5_ebf5);
    assert_eq!(preset.cursor_fg, 0x11_1821);
...
fn slint_terminal_tokens_match_shared_no_frame_defaults() {
    ...
    assert!(
        tokens.contains("terminal-default-bg: dark-mode ? #111821 : #fbfcfe;"),
    ...
    assert!(
        tokens.contains("terminal-cursor-fg: dark-mode ? #111821 : #fbfcfe;")
