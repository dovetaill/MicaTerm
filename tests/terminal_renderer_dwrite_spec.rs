...
fn atlas_and_font_backend_sources_expose_tighter_typography_contract() {
    ...
    assert!(
        font_backend_source.contains("pub const DEFAULT_TERMINAL_LETTER_SPACING_PX: f32 = 1.0;"),
        "shared typography defaults should keep a slightly tightened terminal letter spacing so dense Windows prompt text keeps breathing room without looking over-tracked"
    );
    assert!(
        font_backend_source
