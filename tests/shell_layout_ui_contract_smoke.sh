...
grep -F 'in-out property <bool> workspace-focus-mode: false;' "$APP_WINDOW" >/dev/null
...
grep -F 'callback toggle-workspace-focus-mode-requested();' "$APP_WINDOW" >/dev/null
...
grep -F 'Click to collapse, drag to resize' "$RIGHT_PANEL" >/dev/null
...
grep -F 'in property <bool> workspace-focus-mode: false;' "$TITLEBAR" >/dev/null
grep -F 'callback toggle-workspace-focus-mode-requested;' "$TITLEBAR" >/dev/null
grep -F 'focus-mode-button := TitlebarIconButton {' "$TITLEBAR" >/dev/null
grep -F 'tooltip-text: root.workspace-focus-mode ? "Exit focus mode" : "Enter focus mode";' "$TITLEBAR" >/dev/null
