...
fn bundled_ui_font_assets_exist() {
    assert!(
        ...
        "the shell UI bundle should ship a Sarasa UI SC regular face"
    );
    assert!(
        Path::new("assets/fonts/SarasaUiSC/SarasaUiSC-SemiBold.ttf").exists(),
        "the shell UI bundle should ship a Sarasa UI SC semibold face for emphasis"
    );
    assert!(
        Path::new("assets/fonts/SarasaUiSC/LICENSE.txt").exists(),
        "the shell UI bundle should ship the upstream Sarasa license text"
    );
}
...
fn typography_theme_exposes_the_ui_font_contract() {
    ...
    assert!(source.contains("ui-font-family: \"Sarasa UI SC\";"));
...
fn app_window_uses_sarasa_ui_sc_as_the_shell_default() {
    ...
    assert!(source.contains("import \"../assets/fonts/SarasaUiSC/SarasaUiSC-SemiBold.ttf\";"));
...
fn build_script_tracks_ui_typography_assets() {
    ...
    assert!(source.contains("assets/fonts/SarasaUiSC/SarasaUiSC-Regular.ttf"));
    assert!(source.contains("assets/fonts/SarasaUiSC/SarasaUiSC-SemiBold.ttf"));
}
