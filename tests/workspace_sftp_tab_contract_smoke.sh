...
require_contains 'tooltip-text: "Upload files or folders";' "$SFTP_HOST"
require_contains 'tooltip-text: "Create folder";' "$SFTP_HOST"
require_contains 'tooltip-text: "Open Transfer Center";' "$SFTP_HOST"
require_contains 'content-state-surface := Rectangle {' "$SFTP_HOST"
require_contains 'Last path: ' "$SFTP_HOST"
...
require_absent 'function workspace-toolbar-action-labels-visible() -> bool {' "$SFTP_HOST"
require_absent 'label: root.workspace-toolbar-action-labels-visible() ? "Upload" : "";' "$SFTP_HOST"
require_absent 'label: root.workspace-toolbar-action-labels-visible() ? "New Folder" : "";' "$SFTP_HOST"
require_absent 'label: root.workspace-toolbar-action-labels-visible() ? "Transfer Center" : "";' "$SFTP_HOST"
require_absent 'function statusbar-binding-summary() -> string {' "$SFTP_HOST"
require_absent 'function transfer-entry-width() -> length {' "$SFTP_HOST"
require_absent 'transfer-entry := Rectangle {' "$SFTP_HOST"
require_absent 'return root.width < 1120px;' "$SFTP_HOST"
require_absent 'return "Transfer Center";' "$SFTP_HOST"
require_absent 'overlay-card := Rectangle' "$SFTP_HOST"
