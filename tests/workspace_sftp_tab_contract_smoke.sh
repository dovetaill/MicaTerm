...
require_contains 'function workspace-toolbar-action-labels-visible() -> bool {' "$SFTP_HOST"
require_contains 'return root.workspace-width-tier() == "wide";' "$SFTP_HOST"
require_contains 'transfer-center-button := WorkspaceActionButton {' "$SFTP_HOST"
require_contains 'label: root.workspace-toolbar-action-labels-visible() ? "Upload" : "";' "$SFTP_HOST"
require_contains 'label: root.workspace-toolbar-action-labels-visible() ? "New Folder" : "";' "$SFTP_HOST"
require_contains 'label: root.workspace-toolbar-action-labels-visible() ? "Transfer Center" : "";' "$SFTP_HOST"
require_contains 'function statusbar-binding-summary() -> string {' "$SFTP_HOST"
require_contains 'function transfer-entry-width() -> length {' "$SFTP_HOST"
require_contains 'return root.workspace-width-tier() == "wide" ? "No transfers" : "";' "$SFTP_HOST"
