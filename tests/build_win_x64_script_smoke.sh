...
grep -F "Direct3D-first native terminal renderer package with scene-image presenter default." <<<"$HELP_OUTPUT" >/dev/null
...
grep -F "packaged terminal subsystem: scene-image" <<<"$HELP_OUTPUT" >/dev/null
...
grep -F 'export MICA_TERM_PACKAGE_TERMINAL_SUBSYSTEM="scene-image"' "$SCRIPT_PATH" >/dev/null
