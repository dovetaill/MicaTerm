...
APP_WINDOW=ui/app-window.slint
...
grep -F 'out property <brush> terminal-command-decoration-running:' "$TOKENS"
grep -F 'out property <brush> terminal-command-decoration-failure:' "$TOKENS"
grep -F 'out property <brush> terminal-overview-marker-failure:' "$TOKENS"
...
grep -F 'workspace-session-command-blocks' "$APP_WINDOW"
grep -F 'workspace-session-overview-markers' "$APP_WINDOW"
grep -F 'session-command-blocks: root.workspace-session-command-blocks;' "$WORKSPACE_PANE"
grep -F 'session-overview-markers: root.workspace-session-overview-markers;' "$WORKSPACE_PANE"
grep -F 'ThemeTokens.terminal-command-decoration-running' "$TERMINAL_HOST"
