...
TOOLBAR_BUTTON="$ROOT_DIR/ui/components/sidebar-toolbar-icon-button.slint"
...
grep -F 'callback tooltip-open-requested(string, string, length, length, length);' "$TOOLBAR_BUTTON" >/dev/null
grep -F 'callback tooltip-close-requested(string);' "$TOOLBAR_BUTTON" >/dev/null
grep -F 'in property <string> tooltip-text;' "$TOOLBAR_BUTTON" >/dev/null
