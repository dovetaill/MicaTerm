...
use mica_term::app::logging::runtime::{
    ...
    emit_terminal_memory_cache_clear, emit_terminal_memory_cleanup_result,
    emit_terminal_memory_idle_transition, emit_terminal_memory_surface_transition,
};
...
#[test]
fn terminal_memory_cleanup_result_emits_backend_and_process_snapshot_when_enabled() {
    let temp_root = std::env::temp_dir()
        .join("mica-term")
        .join("tests")
        .join("logging-runtime-memory-cleanup");
    let _ = fs::remove_dir_all(&temp_root);
    fs::create_dir_all(temp_root.join("logs")).unwrap();
    fs::create_dir_all(temp_root.join("crash")).unwrap();
    ...
    tracing::dispatcher::with_default(&runtime.dispatch, || {
        emit_terminal_memory_cleanup_result(
            true,
            "idle-shrink-cleanup",
            "no-active-surface-idle",
            1000,
            ...
            Some(ProcessMemorySnapshot {
                ...
                peak_working_set_bytes: 180 * 1024 * 1024,
                pagefile_usage_bytes: 210 * 1024 * 1024,
                private_usage_bytes: 205 * 1024 * 1024,
            }),
            Some(ProcessMemorySnapshot {
                working_set_bytes: 120 * 1024 * 1024,
                peak_working_set_bytes: 180 * 1024 * 1024,
                pagefile_usage_bytes: 210 * 1024 * 1024,
                private_usage_bytes: 205 * 1024 * 1024,
            }),
    ...
    let content = fs::read_to_string(paths.logs_dir.join("system-error.log")).unwrap();
    assert!(content.contains("terminal memory cleanup result"));
    assert!(content.contains("idle-shrink-cleanup"));
    assert!(content.contains("no_surface_idle_ms=1000"));
    assert!(content.contains("backend_purge_succeeded=false"));
    assert!(content.contains("process_trim_succeeded=false"));
    assert!(content.contains("before_working_set_bytes=167772160"));
    assert!(content.contains("before_private_usage_bytes=214958080"));
    assert!(content.contains("after_working_set_bytes=125829120"));
    assert!(content.contains("after_private_usage_bytes=214958080"));
}

#[test]
fn terminal_memory_idle_transition_emits_when_enabled() {
    let temp_root = std::env::temp_dir()
        .join("mica-term")
        .join("tests")
        .join("logging-runtime-memory-idle-transition");
    let _ = fs::remove_dir_all(&temp_root);
    fs::create_dir_all(temp_root.join("logs")).unwrap();
    fs::create_dir_all(temp_root.join("crash")).unwrap();
    ...
    tracing::dispatcher::with_default(&runtime.dispatch, || {
        emit_terminal_memory_idle_transition(
            true,
            "idle-shrink-armed",
    ...
    let content = fs::read_to_string(paths.logs_dir.join("system-error.log")).unwrap();
    assert!(content.contains("terminal memory idle transition"));
    assert!(content.contains("idle-shrink-armed"));
    assert!(content.contains("reason=\"workspace-tab-close-requested\""));
    assert!(content.contains("no_surface_idle_ms=0"));
    assert!(content.contains("idle_threshold_ms=1000"));
    assert!(content.contains("renderer_resources_retained=true"));
    assert!(content.contains("idle_cache_shrunk=false"));
}

#[test]
fn terminal_memory_surface_transition_emits_when_enabled() {
    let temp_root = std::env::temp_dir()
        .join("mica-term")
        .join("tests")
        .join("logging-runtime-memory-surface-transition");
    let _ = fs::remove_dir_all(&temp_root);
    fs::create_dir_all(temp_root.join("logs")).unwrap();
    fs::create_dir_all(temp_root.join("crash")).unwrap();
    ...
    tracing::dispatcher::with_default(&runtime.dispatch, || {
        emit_terminal_memory_surface_transition(
            true,
    ...
    let content = fs::read_to_string(paths.logs_dir.join("system-error.log")).unwrap();
    assert!(content.contains("terminal memory surface transition"));
    assert!(content.contains("tab-close-transition"));
    assert!(content.contains("reason=\"workspace-close-tab-action\""));
    assert!(content.contains("had_active_surface=true"));
    assert!(content.contains("has_active_surface=false"));
    assert!(content.contains("surface_disappeared=true"));
